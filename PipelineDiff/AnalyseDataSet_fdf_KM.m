function [Dcm enum]= AnalyseDataSet_fdf_KM(varargin)
...
%     enum.dataset(dataset_num).slc=[];
    for cpt=3:1:size(listing,1)
        if listing(cpt).name(end-2:end) == 'dcm' | listing(cpt).name(end-2:end) == 'IMA' | listing(cpt).name(end-2:end) == 'fdf'
            
%            tmpInfoDcm=dicominfo();
            tmpInfoDcm=Header_fdf(listing(cpt).name);
            infoDcm(k).name=listing(cpt).name;
            infoDcm(k).slicePerMosa=1;
            infoDcm(k).tt=0;
            infoDcm(k).slc=tmpInfoDcm.slc;
            infoDcm(k).dirV=tmpInfoDcm.dirV; 
            
            if (sum(round(tmpInfoDcm.dirV))==0)
                infoDcm(k).b=0; %b-value=0
            else
                infoDcm(k).b=tmpInfoDcm.bval; %b-value
            end
            infoDcm(k).slicePerMosa=1;
            enum.mosa=1;
            tmpV(1,:)=[20 20 20 20 20 20 20 20 20 20 20 20 20 20 20];
...
%            enum.dataset(dataset_num).slc(cpt_slc).b(cpt_b).dir(cpt_dir).avg(cpt_avg)=[];
    ...
    %%% Create Dcm File %%%
     ...
     Dcm=zeros(size(double(fdf_KM(infoDcm(1).name)),1),size(double(fdf_KM(infoDcm(1).name)),2), enum.datasize(dataset_num).slc, enum.datasize(dataset_num).b, enum.datasize(dataset_num).dir, enum.datasize(dataset_num).avg);
       ...
       for cpt_b=1:1:enum.datasize(dataset_num).b     
          for cpt_dir=1:1: enum.dataset(dataset_num).slc(cpt_slc).b(cpt_b).nb_dir           
             for cpt_avg=1:1: enum.dataset(dataset_num).slc(cpt_slc).b(cpt_b).dir(cpt_dir).nb_avg
                 % img(:,:,cpt2) = fdf_KM (listing(cpt).name);
                  tmp_dcm=double(fdf_KM(enum.dataset(dataset_num).slc(cpt_slc).b(cpt_b).dir(cpt_dir).avg(cpt_avg).filename));
                  if size(tmp_dcm,1)==size(Dcm,1)
                    Dcm(:,:,cpt_slc,cpt_b,cpt_dir,cpt_avg)=double(fdf_KM(enum.dataset(dataset_num).slc(cpt_slc).b(cpt_b).dir(cpt_dir).avg(cpt_avg).filename));
                  else
                    Dcm(:,:,cpt_slc,cpt_b,cpt_dir,cpt_avg)=double(fdf_KM(enum.dataset(dataset_num).slc(cpt_slc).b(cpt_b).dir(cpt_dir).avg(cpt_avg).filename)'); 
                  end
             end
...
while ischar(tline)
    tline = fgetl(fid);
    tmp=strsplit(tline,' ');
    if cpt_header==16  % Slice number
        header.slc=str2num(string(tmp{4}));
    end
    if cpt_header==42  % b-value number
        header.bval=str2num(string(tmp{4}));
    end
    
    if cpt_header==39  % grad x
        header.dirV(1)=str2num(string(tmp{4}));
    end
    
    if cpt_header==40  % grad y
        header.dirV(2)=str2num(string(tmp{4}));
    end
    
    if cpt_header==41  % grad z
        header.dirV(3)=str2num(string(tmp{4}));
    end
    if cpt_header==45
        break;
    end
