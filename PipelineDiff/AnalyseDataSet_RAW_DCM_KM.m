function [Dcm enum]= AnalyseDataSet_RAW_DCM_KM(varargin)
        ...
        if listing(cpt).name(end-2:end) == 'dcm' | listing(cpt).name(end-2:end) == 'IMA'
            ...
            %if (isfield(tmpInfoDcm, 'Directionality'))  % check if the file is a valid diffusion file
                ...
                if (str2num(sseries{1})~=str2num(sseries{7}))
                        ...
                        infoDcm(k).b = str2num(sseries{5});% tmpInfoDcm.Bvalue;
                        infoDcm(k).dirV= str2num(sseries{4});
                        infoDcm(k).slc = str2num(sseries{2});
                        infoDcm(k).avg = str2num(sseries{3});
