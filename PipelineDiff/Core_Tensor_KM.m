function [Tensor, EigValue,EigVector,MD,FA,Trace_DTI]= Core_Tensor_KM(Dwi,Dir,Bvalue)
...
% Ennis Lab @ Stanford: https://med.stanford.edu/cmrgroup/software.html
 ...
 for cpt_slc=1:1:size(Dwi,3)
  [tmp_Tensor,tmp_EigVector,tmp_EigValue, Mat] = Tensor_local(squeeze(Dwi(:,:,cpt_slc,:)),Dir,Bvalue);

  Tensor(:,:,:,cpt_slc)=tmp_Tensor;
  EigVector(:,:,:,:,cpt_slc)=tmp_EigVector;
  EigValue(:,:,:,cpt_slc)=tmp_EigValue;
  [tmp_MD, tmp_FA, tmp_Trace] = Maps_local(tmp_EigValue); 
  MD(:,:,cpt_slc)=tmp_MD;
  FA(:,:,cpt_slc)=tmp_FA;
  Trace_DTI(:,:,cpt_slc)=tmp_Trace;
 end
