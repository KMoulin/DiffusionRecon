function [Dcm enum]= AnalyseDataSet_Philips_KM(varargin)
...
%     enum.dataset(dataset_num).slc=[];
    for cpt=3:1:size(listing,1)
        if listing(cpt).name(end-2:end) == 'dcm' | listing(cpt).name(end-2:end) == 'IMA'
            ...
            if strcmp(tmpInfoDcm.Private_2005_140f.Item_1.AcquisitionContrast, 'DIFFUSION')  % check if the file is a valid diffusion file
...
%                 infoDcm(k).AcqTime= (str2double(tmpV(1,1:2))*60*60+str2double(tmpV(1,3:4))*60+str2double(tmpV(1,5:6))) + str2double(tmpV(1,8))*10^-1 + str2double(tmpV(1,9))*10^-2 + str2double(tmpV(1,10))*10^-3 + str2double(tmpV(1,11))*10^-4;  %s
               ...
               if  (tmpInfoDcm.Private_2001_1003 == 0)
                   ...
                   infoDcm(k).slc = tmpInfoDcm.SliceLocation; 
                    ...
                    infoDcm(k).dirV= ([Im_1';Im_2';Im_3']*[tmpInfoDcm.Private_2005_10b0 tmpInfoDcm.Private_2005_10b1 tmpInfoDcm.Private_2005_10b2]')';
                  % infoDcm(k).dirV = [tmpInfoDcm.Private_2005_10b0 tmpInfoDcm.Private_2005_10b1 tmpInfoDcm.Private_2005_10b2];%(tmpInfoDcm.DiffusionGradientOrientation)';
               end
