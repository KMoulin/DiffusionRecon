function [Dcm2 enum2]= Depolation_KM(Dcm, enum)
         ...
         for cpt_b=1:1:enum.datasize(cpt_set).b     
           for cpt_dir=1:1: enum.dataset(cpt_set).slc(cpt_slc).b(cpt_b).nb_dir  
                   for cpt_avg=1:1:enum.dataset(cpt_set).slc(cpt_slc).b(cpt_b).dir(cpt_dir).nb_avg
                         ...
                         tmpZero=zeros(size(squeeze(Dcm(:,:,cpt_slc,(cpt_b),cpt_dir,cpt_avg,cpt_set))));
                         tmpKspace=fftshift(fft2(squeeze(Dcm(:,:,cpt_slc,(cpt_b),cpt_dir,cpt_avg,cpt_set))));
                         
                         tmpZero(size(tmpKspace,1)/4:3*size(tmpKspace,1)/4-1,size(tmpKspace,2)/4:3*size(tmpKspace,2)/4-1)=tmpKspace(size(tmpKspace,1)/4:3*size(tmpKspace,1)/4-1,size(tmpKspace,2)/4:3*size(tmpKspace,2)/4-1);
