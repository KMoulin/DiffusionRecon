function[P_Endo,P_Epi,LV_Mask,Mask_Depth]= ROI_ROI_KM(Dcm,ROI)
...
% Ennis Lab @ Stanford: https://med.stanford.edu/cmrgroup/software.html
    ...
    for cpt_slc=1:1:size(Dcm,3)
       [epicardium, endocardium, LV_tmp] = Spline_Segmentation(Dcm(:,:,cpt_slc,1,1,1,1), 30, 30,ROI);
...
function [epi, endo, LV_mask] = Spline_Segmentation(IM, nEpi_max, nEndo_max,ROI)
 
clear 'hepi';
clear 'hendo';
 
figure; 
imagesc(IM,[0 nanmean(nanmean(IM))*5]); hold on; % here just view the image you want to base your borders on min(min(IM)) max(max(IM))
plot(ROI(:,1),ROI(:,2))
plot(ROI(:,3),ROI(:,4))
